...
const LibroEditar = () => {
  ...
  const [form, setForm] = useState({ nombre: "", autor: "", precio: "", categoria: "", imagenUrl: "", isbn: "", urlLibro: "" });
  ...
  const cancelar = () => {
    setEditando(null);
    setForm({ nombre: "", autor: "", precio: "", categoria: "", imagenUrl: "", isbn: "", urlLibro: "" });
    setMensaje("");
  };

  const guardarCambios = () => {
    const libroEditado = form;
    axios.put(`http://localhost:3000/libros/${editando}`, libroEditado)
      .then(res => {
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'32px',justifyContent:'flex-start',marginTop:'24px'}}>
        {librosFiltrados.map((libro, idx) => (
          <div key={idx} style={{width:'180px',display:'flex',flexDirection:'column',alignItems:'center',marginBottom:'24px'}}>
            ...
            <button style={{marginTop:'10px',background:'#1746A2',color:'#fff',border:'none',borderRadius:'4px',padding:'4px 10px',fontWeight:'bold',fontSize:'13px',cursor:'pointer'}} onClick={e => {e.stopPropagation();seleccionarLibro(idx);}}>Editar</button>
          </div>
        ))}
      </div>
      {editando !== null && (
        <div style={{position:'fixed',top:0,left:0,width:'100vw',height:'100vh',background:'rgba(0,0,0,0.2)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000}}>
          <div style={{background:'#fff',padding:'32px 24px',borderRadius:'8px',boxShadow:'0 2px 8px rgba(0,0,0,0.15)',minWidth:'320px',textAlign:'center'}}>
            ...
            <input name="autor" value={form.autor} onChange={handleChange} placeholder="Autor" style={{marginBottom:'8px',width:'90%',padding:'8px'}} />
            <input name="isbn" value={form.isbn} onChange={handleChange} placeholder="ISBN" style={{marginBottom:'8px',width:'90%',padding:'8px'}} />
            <input name="urlLibro" value={form.urlLibro} onChange={handleChange} placeholder="URL libro (PDF)" style={{marginBottom:'8px',width:'90%',padding:'8px'}} />
            <input name="precio" value={form.precio} onChange={handleChange} placeholder="Precio" type="number" style={{marginBottom:'8px',width:'90%',padding:'8px'}} />
            <select name="categoria" value={form.categoria} onChange={handleChange} style={{marginBottom:'8px',width:'90%',padding:'8px'}}>
